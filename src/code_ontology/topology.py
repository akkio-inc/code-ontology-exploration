...
def compute_temporal_patterns(commits: list[dict]) -> dict:
    """Extract temporal commit patterns: hour-of-day, day-of-week, heatmap, message analysis."""
    import re
    from datetime import datetime
    ...
    has_coauthor_count = 0
    has_footer_count = 0
    msg_line_counts: list[int] = []

    # Conventional commit prefixes (feat:, fix:, chore:, docs:, etc.)
    conventional_re = re.compile(r"^(feat|fix|chore|docs|style|refactor|perf|test|build|ci|revert)(\(.+\))?!?:", re.I)
    coauthor_re = re.compile(r"Co-authored-by:", re.I)
    footer_re = re.compile(r"(Signed-off-by:|Reviewed-by:|Acked-by:|Tested-by:|Agent-Logs-Url:|Generated by)", re.I)

    # Per-author-category temporal patterns
    cat_heatmaps: dict[str, list[list[int]]] = {
        "human": [[0] * 7 for _ in range(24)],
        "ai_agent": [[0] * 7 for _ in range(24)],
        "automation_bot": [[0] * 7 for _ in range(24)],
    }
    cat_hour_counts: dict[str, list[int]] = {
        "human": [0] * 24,
        "ai_agent": [0] * 24,
        "automation_bot": [0] * 24,
    }
    ...
    for c in commits:
        ...
        msg_line_counts.append(len(lines))
        if len(lines) > 1:
            multiline_count += 1
        if conventional_re.match(msg):
            conventional_count += 1
        if coauthor_re.search(msg):
            has_coauthor_count += 1
        if footer_re.search(msg):
            has_footer_count += 1

        # Per-category temporal
        cat = c.get("author_category", "human")
        if cat in cat_heatmaps:
            cat_heatmaps[cat][hour][dow] += 1
            cat_hour_counts[cat][hour] += 1
    ...
    return {
        ...
        # New commit message metrics
        "commit_message_metrics": {
            "multiline_ratio": round(multiline_count / n, 4),
            "conventional_commit_ratio": round(conventional_count / n, 4),
            "coauthor_ratio": round(has_coauthor_count / n, 4),
            "footer_ratio": round(has_footer_count / n, 4),
            "avg_lines_per_message": round(sum(msg_line_counts) / n, 2) if msg_line_counts else 0,
            "multiline_count": multiline_count,
            "conventional_count": conventional_count,
            "coauthor_count": has_coauthor_count,
            "footer_count": has_footer_count,
        },
        # Per-author-category temporal breakdown
        "by_author_category": {
            cat: {
                "hour_of_day": [{"hour": h, "count": ct} for h, ct in enumerate(cat_hour_counts[cat])],
                "heatmap": cat_heatmaps[cat],
            }
...
def compute_topology(commits: list[dict], graph: nx.Graph) -> dict:
    ...
    # Ensure commits have author classification for temporal breakdown
    from code_ontology.author_classifier import classify_commits
